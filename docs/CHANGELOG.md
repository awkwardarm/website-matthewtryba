...
- Removed the "Enjoying These Tools?" section (tier list, general donate widget, and recurring-donation link); each tool's widget now reads "Donate for {tool}." with "Secure checkout by Stripe." on the next line.
...
- **Why:** the client-facing documents (rate cards, service menus, prep checklists, FAQs) were authored in Claude Design and sent as PDF attachments. Every edit meant re-export, re-download, re-send — and superseded PDFs live in clients' inboxes forever, so there was no single source of truth. They are now unlisted pages: send the link once, and it always shows the current version. **The repo is now the source of truth**; Claude Design is a drafting tool you pull redesigns from.
- **Ten documents** at `/docs/<slug>/` — Services & Pricing, Studio Rates, Production Pathway, Creative Date Checklist (In Studio), Remote Vocal Recording, Remote Vocal Tech Setup, and four FAQs (PROs/copyrights/splits, distribution, promotion, sync). `/docs/` is an unlisted index of all of them, ordered to match `docs/CLIENT-DOCUMENT-RELEASE-ORDER.md`.
- **Unlisted, not private.** `src/docs/docs.11tydata.js` sets `noindex: true` for the whole directory, so these stay out of search engines and out of `sitemap.xml` — but a link can still be forwarded. The Profit & Loss statement was deliberately **not** migrated for this reason.
- **`assets/doc.css`** consolidates the near-identical `<style>` block each Design export carried inline: the paper column, the print running header/footer scaffold, the FAQ card components, and IBM Plex Mono (Outfit already came from `shared-styles.css`). Scoped under `.doc`, and loaded only when a page sets `docPage: true`. The print block hides site chrome so Cmd+P → Save as PDF still produces the original document — one source, both outputs.
- **`base.njk`** gained a `docPage` conditional: the stylesheet link, a screen-only "Save as PDF" + last-updated bar, a `doc-page` body class, and a guard that skips the persistent now-playing audio bar on documents.
- **`scripts/import-design-doc.py`** re-imports a redesign from a Claude Design export — strips the `<x-dc>`/`<helmet>` wrapper, converts `<image-slot>` custom elements to plain `<img>`, drops the pixel width/height the canvas editor stamps on resized elements (one had a paragraph pinned to 589px, which broke mobile layout), and rewrites asset paths to `/images/docs/`.
- **`templateEngineOverride: false`** on the directory so a literal `{{` in future document text can't break the build; the permalink is a function and the last-updated date renders in the layout as a result.
- Document images added under `images/docs/` (~684KB).
- **Production Pathway reflows to cards on phones.** The roadmap's 8x4 grid cells are now grouped into row wrappers carrying `display: contents`, which removes them from layout entirely — so the desktop grid and the printed landscape sheet are untouched. Below 720px the wrappers become one card per phase: phase name as the heading, then each populated column beneath it with a small label, empty cells hidden, and the deepening blue kept (each cell already carries its row's background). Considered rendering it as an image instead and rejected it: the table carries three live links including the DistroKid referral, and an image would also reintroduce the export-and-sync loop this whole change removes.
- **Narrow-screen fixes.** These layouts were built for 8.5in paper, and two patterns don't survive a phone column: a label-and-price row (the label is elastic, the price is `white-space:nowrap`, so the price paints out through the card's right edge), and the Production Pathway's four-column roadmap grid (which sets `overflow:hidden`, so squeezing it silently clips text). Rows now wrap — unscoped, since flex only breaks a line when content genuinely doesn't fit — and the roadmap scrolls inside its own container below 720px instead of being crushed. `.doc-frame` also switched to `table-layout: fixed` on screen so the single-column print scaffold obeys its container instead of growing to its widest child. Print output verified unchanged throughout: PDFs for five documents are identical in size before and after, differing only in their embedded `CreationDate`.
